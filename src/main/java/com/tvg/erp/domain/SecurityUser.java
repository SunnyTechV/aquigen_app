...
public class SecurityUser implements Serializable {
    ...
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(
        value = { "purchaseOrderDetails", "rawMaterialOrders", "categories", "unit", "securityUser" },
        allowSetters = true
    )
    private Set<RawMaterialMaster> rawMaterialMasters = new HashSet<>();

    @OneToMany(mappedBy = "securityUser")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(
        value = { "categories", "unit", "quatationDetails", "securityUser", "productInventories", "productTransactions" },
        allowSetters = true
    )
    ...
    public Set<RawMaterialMaster> getRawMaterialMasters() {
        return this.rawMaterialMasters;
    }

    public void setRawMaterialMasters(Set<RawMaterialMaster> rawMaterialMasters) {
        if (this.rawMaterialMasters != null) {
            this.rawMaterialMasters.forEach(i -> i.setSecurityUser(null));
        }
        if (rawMaterialMasters != null) {
            rawMaterialMasters.forEach(i -> i.setSecurityUser(this));
        }
        this.rawMaterialMasters = rawMaterialMasters;
    }

    public SecurityUser rawMaterialMasters(Set<RawMaterialMaster> rawMaterialMasters) {
        this.setRawMaterialMasters(rawMaterialMasters);
        return this;
    }

    public SecurityUser addRawMaterialMaster(RawMaterialMaster rawMaterialMaster) {
        this.rawMaterialMasters.add(rawMaterialMaster);
        rawMaterialMaster.setSecurityUser(this);
        return this;
    }

    public SecurityUser removeRawMaterialMaster(RawMaterialMaster rawMaterialMaster) {
        this.rawMaterialMasters.remove(rawMaterialMaster);
        rawMaterialMaster.setSecurityUser(null);
        return this;
    }
