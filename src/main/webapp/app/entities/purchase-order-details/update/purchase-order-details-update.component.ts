...
export class PurchaseOrderDetailsUpdateComponent implements OnInit {
  ...
  rawMaterialMastersSharedCollection: IRawMaterialMaster[] = [];
  ...
  trackRawMaterialMasterById(index: number, item: IRawMaterialMaster): number {
    return item.id!;
  }
  ...
  protected updateForm(purchaseOrderDetails: IPurchaseOrderDetails): void {
    this.editForm.patchValue({
      ...
      rawMaterialMaster: purchaseOrderDetails.rawMaterialMaster,
    ...
    this.rawMaterialMastersSharedCollection = this.rawMaterialMasterService.addRawMaterialMasterToCollectionIfMissing(
      this.rawMaterialMastersSharedCollection,
      purchaseOrderDetails.rawMaterialMaster
    );
  ...
  protected loadRelationshipsOptions(): void {
    ...
    this.rawMaterialMasterService
      .query()
      .pipe(map((res: HttpResponse<IRawMaterialMaster[]>) => res.body ?? []))
      .pipe(
        map((rawMaterialMasters: IRawMaterialMaster[]) =>
          this.rawMaterialMasterService.addRawMaterialMasterToCollectionIfMissing(
            rawMaterialMasters,
            this.editForm.get('rawMaterialMaster')!.value
          )
        )
      )
      .subscribe((rawMaterialMasters: IRawMaterialMaster[]) => (this.rawMaterialMastersSharedCollection = rawMaterialMasters));
  ...
  protected createFromForm(): IPurchaseOrderDetails {
    return {
      ...
      rawMaterialMaster: this.editForm.get(['rawMaterialMaster'])!.value,
