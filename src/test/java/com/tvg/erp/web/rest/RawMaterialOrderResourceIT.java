...
import com.tvg.erp.repository.RawMaterialOrderRepository;
import com.tvg.erp.service.criteria.RawMaterialOrderCriteria;
import com.tvg.erp.service.dto.RawMaterialOrderDTO;
import com.tvg.erp.service.mapper.RawMaterialOrderMapper;
...
class RawMaterialOrderResourceIT {
    ...
    @Test
    @Transactional
    void getAllRawMaterialOrdersByRawMaterialMasterIsEqualToSomething() throws Exception {
        // Initialize the database
        rawMaterialOrderRepository.saveAndFlush(rawMaterialOrder);
        ...
        em.persist(rawMaterialMaster);
        em.flush();
        rawMaterialOrder.addRawMaterialMaster(rawMaterialMaster);
        rawMaterialOrderRepository.saveAndFlush(rawMaterialOrder);
        Long rawMaterialMasterId = rawMaterialMaster.getId();

        // Get all the rawMaterialOrderList where rawMaterialMaster equals to rawMaterialMasterId
        defaultRawMaterialOrderShouldBeFound("rawMaterialMasterId.equals=" + rawMaterialMasterId);

        // Get all the rawMaterialOrderList where rawMaterialMaster equals to (rawMaterialMasterId + 1)
        defaultRawMaterialOrderShouldNotBeFound("rawMaterialMasterId.equals=" + (rawMaterialMasterId + 1));
    }
