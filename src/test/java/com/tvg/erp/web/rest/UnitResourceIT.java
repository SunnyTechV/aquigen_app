...
import com.tvg.erp.repository.UnitRepository;
import com.tvg.erp.service.criteria.UnitCriteria;
import com.tvg.erp.service.dto.UnitDTO;
import com.tvg.erp.service.mapper.UnitMapper;
...
class UnitResourceIT {
    ...
    @Test
    @Transactional
    void getAllUnitsByRawMaterialMasterIsEqualToSomething() throws Exception {
        // Initialize the database
        unitRepository.saveAndFlush(unit);
        ...
        em.persist(rawMaterialMaster);
        em.flush();
        unit.addRawMaterialMaster(rawMaterialMaster);
        unitRepository.saveAndFlush(unit);
        Long rawMaterialMasterId = rawMaterialMaster.getId();

        // Get all the unitList where rawMaterialMaster equals to rawMaterialMasterId
        defaultUnitShouldBeFound("rawMaterialMasterId.equals=" + rawMaterialMasterId);

        // Get all the unitList where rawMaterialMaster equals to (rawMaterialMasterId + 1)
        defaultUnitShouldNotBeFound("rawMaterialMasterId.equals=" + (rawMaterialMasterId + 1));
    }
