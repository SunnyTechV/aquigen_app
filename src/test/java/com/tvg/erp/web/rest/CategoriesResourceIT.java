...
import com.tvg.erp.repository.CategoriesRepository;
import com.tvg.erp.service.criteria.CategoriesCriteria;
import com.tvg.erp.service.dto.CategoriesDTO;
import com.tvg.erp.service.mapper.CategoriesMapper;
...
class CategoriesResourceIT {
    ...
    @Test
    @Transactional
    void getAllCategoriesByRawMaterialMasterIsEqualToSomething() throws Exception {
        // Initialize the database
        categoriesRepository.saveAndFlush(categories);
        ...
        em.persist(rawMaterialMaster);
        em.flush();
        categories.addRawMaterialMaster(rawMaterialMaster);
        categoriesRepository.saveAndFlush(categories);
        Long rawMaterialMasterId = rawMaterialMaster.getId();

        // Get all the categoriesList where rawMaterialMaster equals to rawMaterialMasterId
        defaultCategoriesShouldBeFound("rawMaterialMasterId.equals=" + rawMaterialMasterId);

        // Get all the categoriesList where rawMaterialMaster equals to (rawMaterialMasterId + 1)
        defaultCategoriesShouldNotBeFound("rawMaterialMasterId.equals=" + (rawMaterialMasterId + 1));
    }
