...
import com.tvg.erp.repository.ConsumptionDetailsRepository;
import com.tvg.erp.service.criteria.ConsumptionDetailsCriteria;
import com.tvg.erp.service.dto.ConsumptionDetailsDTO;
import com.tvg.erp.service.mapper.ConsumptionDetailsMapper;
...
class ConsumptionDetailsResourceIT {
    ...
    @Test
    @Transactional
    void getAllConsumptionDetailsByRmInventoryIsEqualToSomething() throws Exception {
        // Initialize the database
        consumptionDetailsRepository.saveAndFlush(consumptionDetails);
        ...
        em.persist(rmInventory);
        em.flush();
        consumptionDetails.setRmInventory(rmInventory);
        consumptionDetailsRepository.saveAndFlush(consumptionDetails);
        Long rmInventoryId = rmInventory.getId();

        // Get all the consumptionDetailsList where rmInventory equals to rmInventoryId
        defaultConsumptionDetailsShouldBeFound("rmInventoryId.equals=" + rmInventoryId);

        // Get all the consumptionDetailsList where rmInventory equals to (rmInventoryId + 1)
        defaultConsumptionDetailsShouldNotBeFound("rmInventoryId.equals=" + (rmInventoryId + 1));
    }
