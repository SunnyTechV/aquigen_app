...
import com.tvg.erp.repository.PurchaseOrderRepository;
import com.tvg.erp.service.criteria.PurchaseOrderCriteria;
import com.tvg.erp.service.dto.PurchaseOrderDTO;
import com.tvg.erp.service.mapper.PurchaseOrderMapper;
...
class PurchaseOrderResourceIT {
    ...
    @Test
    @Transactional
    void getAllPurchaseOrdersByRmInventoryIsEqualToSomething() throws Exception {
        // Initialize the database
        purchaseOrderRepository.saveAndFlush(purchaseOrder);
        ...
        em.persist(rmInventory);
        em.flush();
        purchaseOrder.setRmInventory(rmInventory);
        purchaseOrderRepository.saveAndFlush(purchaseOrder);
        Long rmInventoryId = rmInventory.getId();

        // Get all the purchaseOrderList where rmInventory equals to rmInventoryId
        defaultPurchaseOrderShouldBeFound("rmInventoryId.equals=" + rmInventoryId);

        // Get all the purchaseOrderList where rmInventory equals to (rmInventoryId + 1)
        defaultPurchaseOrderShouldNotBeFound("rmInventoryId.equals=" + (rmInventoryId + 1));
    }
