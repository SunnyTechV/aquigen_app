...
import com.tvg.erp.repository.PurchaseOrderDetailsRepository;
import com.tvg.erp.service.criteria.PurchaseOrderDetailsCriteria;
import com.tvg.erp.service.dto.PurchaseOrderDetailsDTO;
import com.tvg.erp.service.mapper.PurchaseOrderDetailsMapper;
...
class PurchaseOrderDetailsResourceIT {
    ...
    @Test
    @Transactional
    void getAllPurchaseOrderDetailsByRawMaterialMasterIsEqualToSomething() throws Exception {
        // Initialize the database
        purchaseOrderDetailsRepository.saveAndFlush(purchaseOrderDetails);
        RawMaterialMaster rawMaterialMaster;
        if (TestUtil.findAll(em, RawMaterialMaster.class).isEmpty()) {
            rawMaterialMaster = RawMaterialMasterResourceIT.createEntity(em);
            em.persist(rawMaterialMaster);
            em.flush();
        } else {
            rawMaterialMaster = TestUtil.findAll(em, RawMaterialMaster.class).get(0);
        }
        em.persist(rawMaterialMaster);
        em.flush();
        purchaseOrderDetails.setRawMaterialMaster(rawMaterialMaster);
        purchaseOrderDetailsRepository.saveAndFlush(purchaseOrderDetails);
        Long rawMaterialMasterId = rawMaterialMaster.getId();

        // Get all the purchaseOrderDetailsList where rawMaterialMaster equals to rawMaterialMasterId
        defaultPurchaseOrderDetailsShouldBeFound("rawMaterialMasterId.equals=" + rawMaterialMasterId);

        // Get all the purchaseOrderDetailsList where rawMaterialMaster equals to (rawMaterialMasterId + 1)
        defaultPurchaseOrderDetailsShouldNotBeFound("rawMaterialMasterId.equals=" + (rawMaterialMasterId + 1));
    }
